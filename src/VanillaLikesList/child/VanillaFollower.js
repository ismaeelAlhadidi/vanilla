    ...
    connectedCallback() {
        this.innerHTML = `
            <div id="Vanilla${ this.listId }Like${ this.like.id }" class="vanilla-like">
                ...
                <div><button id="Vanilla${ this.listId }Like${ this.like.id }ToggleFollow" style="${ ( this.like.following ? '' : 'font-size: 12px;') }" class="${ ( this.like.following ? 'following' : '' ) }">${ ( this.like.following ? 'Un Follow' : 'Follow Back') }</button></div>
            </div>
        `;
